...
class _ToursScreenState extends State<ToursScreen> {
  ...
  Future<void> _onSearchChanged(String query) async {
    ...
    try {
      ...
      setState(() {
        _tours = _allTours.where((t) =>
            (t['name'] ?? '').toLowerCase().contains(q) ||
            (t['loc'] ?? '').toLowerCase().contains(q)).toList();
      });
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: () => setSheet(() { tempMin = 0; tempMax = 1000; tempSort = 'rating'; }),
                    child: const Text('Reset', style: TextStyle(color: AppColors.gold, fontSize: 13)),
                  ),
                ],
              ),
              ...
              // Price Range
              const Text('Price Range', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('\$${tempMin.toInt()}', style: const TextStyle(color: AppColors.gold, fontSize: 13)),
                  Text('\$${tempMax.toInt()}', style: const TextStyle(color: AppColors.gold, fontSize: 13)),
                ],
              ),
              RangeSlider(
              ...
              // Sort By
              const Text('Sort By', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    setState(() { _minPrice = tempMin; _maxPrice = tempMax; _sortBy = tempSort; });
                    Navigator.pop(ctx);
                    _applyFilters();
                  },
  ...
  void _applyFilters() {
    ...
    results = results.where((t) {
      final price = double.tryParse(
              t['price']?.toString().replaceAll(RegExp(r'[^\d.]'), '') ?? '0') ?? 0;
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    return _tours.where((t) {
      final d = (t['days'] is int) ? t['days'] as int : int.tryParse(t['days'].toString()) ?? 1;
              ...
              child: Column(
                children: [
                        ...
                        if (_isSearching)
                          GestureDetector(
                            onTap: () {
                              ...
                              setState(() { _isSearching = false; _tours = _allTours; });
                            },
                            child: const Padding(
                  ...
                  Align(
                    alignment: Alignment.centerLeft,
                    child: Text(
                      _isLoading ? 'Loading...' : '${_filtered.length} Tours Found',
